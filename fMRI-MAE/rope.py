...
class RotaryPositionalEmbeddings4D(nn.Module):
    ...
    def _build_cache_1d(self, effective_d: int, seq_idx: torch.Tensor)->torch.Tensor:
        ...
        theta = 1. / (self.base ** (torch.arange(0, effective_d, 2).float() / effective_d))
    ...
    def _build_cache(self, x: torch.Tensor)->None:
        ...
        grid = torch.meshgrid(grid_t, grid_d, grid_h, grid_w, indexing="xy") # This order should match with i/p
        grid = torch.stack(grid, axis=0)
        grid = grid.reshape([4, 1, self.grid_time, self.grid_depth, self.grid_height, self.grid_width])
        ...
        emb = torch.concatenate([emb_t, emb_d, emb_h, emb_w], axis=1)  # (T*D*H*W, embedding_dim)
        emb = emb.to(x.device)
        # cache sin and cos
        self.cos_cached = emb.cos()[None, None, :, :] # batch, Num Heads, Seq Len, Embed Dim
    ...
    def forward(self, x: torch.Tensor, mask: Optional[torch.Tensor]):
        ...
        x_rope = (x_rope * self.cos_cached[:, :, :x.shape[1], :]
                 ) + (neg_half_x * self.sin_cached[:, :, :x.shape[1], :]) if mask is None else (
                     x_rope * self.cos_cached[:, :, mask, :] + neg_half_x * self.sin_cached[:, :, mask, :]
                 )
...
if __name__ == "__main__":
    ...
    rot_embed = RotaryPositionalEmbeddings4D(d=512, grid_depth=8, grid_height=8, grid_width=6, grid_time=4)
    query_ = torch.randn(10, 1, 8*8*6*4, 512) # Batch, Heads, Num Tokens/Seq Length, Embedding Dims
